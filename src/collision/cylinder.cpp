...
void Cylinder::collide(PointMass &pm)
{
  ...
  for (int index = 0; index < points.size(); index++)
  {
    ...
    Vector3f p1 = convert(m2, convert(m1, Vector3f(point.x, l, point.z)));
    Vector3f p6 = convert(m2, convert(m1, Vector3f(point.x, -l-point.y, point.z)));
    ax1 = min(p1[0], p6[0]) - r;
    ax2 = max(p1[0], p6[0]) + r;
    ay1 = min(p1[0], p6[0]);
    ay2 = max(p1[0], p6[0]);
...
void Cylinder::render(GLShader &shader)
{
  ...
  for (int index = 0; index < points.size(); index++)
  {
    ...
    for (int i = 0; i < slices; i++)
    {
      ...
      CGL::Matrix3x3 m2 = CGL::Matrix3x3(data2);
      Vector3f p1 = convert(m2, convert(m1, Vector3f(point.x, l, point.z)));
      Vector3f p2 = convert(m2, convert(m1, Vector3f(point.x + r * cos(theta), l, point.z + r * sin(theta))));
      Vector3f p3 = convert(m2, convert(m1, Vector3f(point.x + r * cos(nextTheta), l, point.z + r * sin(nextTheta))));
      Vector3f p4 = convert(m2, convert(m1, Vector3f(point.x + r * cos(nextTheta), -(l + point.y), point.z + r * sin(nextTheta))));
      Vector3f p5 = convert(m2, convert(m1, Vector3f(point.x + r * cos(theta), -(l + point.y), point.z + r * sin(theta))));
      Vector3f p6 = convert(m2, convert(m1, Vector3f(point.x, -(l + point.y), point.z)));
